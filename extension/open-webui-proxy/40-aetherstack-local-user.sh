...
if [ -z "$email" ] && [ -f "$database" ]; then
  ...
  if [ "$admin_count" = "1" ]; then
    ...
    echo "Multiple Open WebUI admins found. Set AETHER_LOCAL_WEBUI_EMAIL." >&2
    exit 1
  fi
fi
email=${email:-local@aetherstack.invalid}
...
sed "s|__AETHER_LOCAL_EMAIL__|$escaped_email|g" /etc/nginx/aetherstack.conf.template > /etc/nginx/conf.d/default.conf
