...
# Marketplace:
code --install-extension AetherStack.aetherstack
# or the exact VSIX downloaded from the v0.3.20 GitHub Release:
code --install-extension aetherstack-0.3.20.vsix
...
Hub `/api/inference/status` merges LiteLLM callbacks with Hub-owned host-CLI call state. Chat shows **Answering: model** while inference runs (setting `aetherstack.showActiveModel`). Telemetry contains only call id, model alias, execution source, state, and timestamps.

Chat defaults to **Auto**. The `/` menu is minimal: **New chat**, **Mode**, **Services**, **Settings**, **Refresh**, and **Recent**. Advanced display options live in VS Code Settings. Slash commands (`/auto`, `/research`, `/plan`, `/code`, `/test`, `/bugfix`, `/help`) still work in the prompt. The transcript survives view restoration and fenced code renders safely.
